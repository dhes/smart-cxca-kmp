...
class CqlExpressionEvaluatorTest {
  ...
  /**
   * Not an assertion — a window. Prints the $apply output (CarePlan + contained
   * CommunicationRequest) as FHIR JSON so a human can examine the in-memory instance:
   * `./gradlew :cxca-cql:desktopTest --tests '*printsGeneratedCommunicationRequest*'`
   */
  @Test
  fun printsGeneratedCommunicationRequest() = runTest {
    val repository = MapRepository()
    repository.create(resource(planDefinition))
    repository.create(resource(activityDefinition))
